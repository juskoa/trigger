...
set ( DOCBOOK_XSL_PREFIX "http://docbook.sourceforge.net/release/xsl-ns/${DOCBOOK_XSL_VERSION}"
      CACHE STRING "prefix to locate the docbook-XSL release files" )
mark_as_advanced ( DOCBOOK_XSL_PREFIX )

function ( _DOCBOOK_GET_HTML_MAN_FILES inFile outFileList )
...
function ( _DOCBOOK_GET_HTML_FILES inFile outFileList )
  ...
  file ( READ "${inFile}" XML_FILE_CONTENTS )
  string ( REPLACE ";" "" XML_FILE_CONTENTS "${XML_FILE_CONTENTS}" )
  string ( REGEX MATCHALL "<chapter[ ]+[^>]*" XML_REFENTRYTITLE "${XML_FILE_CONTENTS}" )
  foreach ( id ${XML_REFENTRYTITLE} )
    string ( REGEX REPLACE ".*id=\"([^\"]*)\".*" "\\1" id "${id}" )
    if ( "${id}" STREQUAL "" )
      message ( FATAL_ERROR "At least one chapter in file ${infile} has no or an empty id attribute." )
    endif ( "${id}" STREQUAL "" )
    list ( APPEND FILES ${id}.html )
  endforeach ( id )
  string ( REGEX MATCHALL "<sect1[ ]+[^>]*" XML_REFENTRYTITLE "${XML_FILE_CONTENTS}" )
  foreach ( id ${XML_REFENTRYTITLE} )
    string ( REGEX REPLACE ".*id=\"([^\"]*)\".*" "\\1" id "${id}" )
#     if ( "${id}" STREQUAL "" )
#       message ( FATAL_ERROR "At least one chapter in file ${infile} has no or an empty id attribute." )
...
#   string ( REGEX MATCH "<!DOCTYPE[ ]+[^ >]*" XML_ENTRY_ELEMENT "${XML_FILE_CONTENTS}" )
#   string ( REGEX REPLACE "<!DOCTYPE[ ]+(.*)" "\\1" XML_ENTRY_ELEMENT "${XML_ENTRY_ELEMENT}" )
  string ( REGEX MATCHALL "<book[ ]+[^>]*" XML_REFENTRYTITLE "${XML_FILE_CONTENTS}" )
  if ( XML_REFENTRYTITLE  )
    list ( APPEND FILES index.html )
  endif ( XML_REFENTRYTITLE )
  set ( ${outFileList} ${FILES} PARENT_SCOPE )
endfunction ( )
...
function ( _DOCBOOK_MAN_HTML inFile outList )
  _docbook_get_html_man_files ( "${inFile}" FILES )
...
#     "html.stylesheet=docbook.css"
  list ( APPEND XSLT_PARAMS_STR
    "section.autolabel=1"
    "section.label.includes.component.label=1"
  )

  CONFIGURE_FILE( ${CMAKE_SOURCE_DIR}/doc/config/doc-html.xsl.in ${CMAKE_CURRENT_BINARY_DIR}/doc-html.xsl )

  xsl_transform (
    "${CMAKE_CURRENT_BINARY_DIR}/doc-html.xsl"
    "${inFile}"
    ${FILES}
  )
#   xsl_transform (
...
function ( DOCBOOK_GENERATE format inFile outList )
  ...
  elseif ( format STREQUAL "manhtml" )
    _docbook_man_html ( "${inFile}" ${outList} )
