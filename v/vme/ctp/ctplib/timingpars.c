...
2. Compilation only for libctp.a: see makefile
   setTimeParsDB()  -to be modified for use with DB
...
void setTimeParsDB(int nTL1, int nTL2, int nTBCL0, 
  int nCALIBRATION_BC, int nORBITLENGTH) {
/* change defaults:
...
TL2= nTL2;
TBCL0= nTBCL0;
CALIBRATION_BC= nCALIBRATION_BC;
ORBITLENGTH= nORBITLENGTH;
}
/*--------------------------------*/ w32 calcPP_TIME(int ppl0) {
// ppl0: time between PP and L0
return(CALIBRATION_BC-66-ppl0);
}
...
if(dt<T12) {
  CLKfactor= rounddown(2*dt/256.); CLKperiod= CLKfactor+1;
  //printf("CLKfactorL%c:%d\n", c12, CLKfactor);
  ablut[ix12+2]= (CLKfactor<<13) | (CLKfactor<<8);
  pfinterval= rounddown(2.0*dt/CLKperiod); pfinterval= pfinterval - 1;
  pfdelay= rounddown(1.0*(dt + T12)/CLKperiod); pfdelay= pfdelay - pfinterval-2;
  ...
  CLKfactor= rounddown((dt+T12)/256.); CLKperiod= CLKfactor+1;
  //printf("CLKfactorL%c:%d\n", c12, CLKfactor);
  ablut[ix12+2]= (CLKfactor<<13) | (CLKfactor<<8);
  pfinterval= rounddown(1.0*(dt+T12)/CLKperiod); pfinterval= pfinterval - 1;
  pfdelay= 0;
...
if(dt>TL2) {
  ...
  CLKfactor= rounddown(dt/256.); CLKperiod= CLKfactor+1;
  //printf("CLKfactorL0:%d\n", CLKfactor);
  ablut[2]= (CLKfactor<<13) | (CLKfactor<<8);
  pfinterval= rounddown(1.0*dt/CLKperiod); pfinterval= (pfinterval-1)&0xff;
