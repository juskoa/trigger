...
#include "ssmctp.h"
//--------------------------------------------------------------------
// Analysis of snapshot memory of INT board
//--------------------------------------------------------------------
...
void printIRDa(IRDa *item,FILE *file){
 ...
 while(item->Inter[i]){
      fprintf(file," %i  %i  BC=%4i 0x%3x \n",i,item->Inter[i],item->bc[i],item->bc[i]);
      printf(" %i  %i  BC=%4i 0x%3x \n",i,item->Inter[i],item->bc[i],item->bc[i]);
      //printf("  %i  0x%x \n",i,item->Inter[i]);
      i++;
 }
}
...
//
CTPRIRDList *addIRDa(CTPRIRDList *last,IRDa *prev){
 CTPRIRDList *p;
 p = (CTPRIRDList *) malloc(sizeof(CTPRIRDList));
 if(!p) goto ERR;
 p->irda = (IRDa *) malloc(sizeof(IRDa));
 if(!(p->irda)) goto ERR;
 copyIRDa(p->irda,prev);
 p->ctpr=NULL;
 p->next=NULL;
 if(last){
  last->next=p;
...
//Print BC numbers in INT records
int bc[3564];
void initprintBCs(){
 int i;
 for(i=0;i<3564;i++)bc[i]=0;
}
void printBCs(CTPRIRDList *list,FILE *file){
 int i;
 IRDa *item;
 if(list == NULL) return;
 //fprintf(file,"BC at INT:\n");
 list=list->first;
 while(list){
   if((item=list->irda)){ 
     i=0;
     while(item->Inter[i]){
      bc[item->bc[i]]=bc[item->bc[i]]+1;
      //printf("--->%i\n",item->bc[i]);
      i++;
     }
   }
 ...
 for(i=0;i<3564;i++){
  if(bc[i]) fprintf(file,"BC at INT:%4i %i\n",i,bc[i]);
 }
}
//PrintCTPRIRDlist
...
/*---------------------------------------------------- startRead3SSM() */
int startRead3SSM() {
int rc,ix,boards[]={4,2,3};   // int l1 l2
for(ix=0; ix<3; ix++) {
  int cb;
  cb= boards[ix]; rc= setomSSM(cb, 0x102); // record 27ms
  if(rc!=0) { printf("Cannot set 0x102 for %d board\n", cb); return(rc); };
};
rc= startSSM(3, boards);
if(rc!=0) { printf("startSSM rc: %d\n", rc); return(rc); };
usleep(28000);
for(ix=0; ix<3; ix++) {
  int cb;
  cb= boards[ix]; readSSM(cb); // read
  if(rc!=0) { printf("readSSM(%d) rc: %d\n", cb, rc); return(rc); };
};
return(0);
}
/*--------------------------------------------------getL2alist()
...
CTPRIRDList *getCTPRIRDList(int boardint,CTPRIRDList *list){
 ...
 while(i<Mega){     
   // Find  end of block
   if(bit(sms[boardint].sm[i],sctrl)){
    ...
    if(bit(sms[boardint].sm[i],sten)){
      blockid=bit(sms[boardint].sm[i],sblockid);
...
CTPRIRDList *getL1IRList(int offset,int board,CTPRIRDList *list){
 ...
 while(i<Mega){
  ...
  int1=bit(sms[board].sm[i],sint1); 
  int2=bit(sms[board].sm[i],sint2); 
  if(int1 || int2){
