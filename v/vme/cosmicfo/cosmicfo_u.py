...
def RandomGenerators(vb):
  class rg:
    def __init__(self, vb):
      ...
      self.srbe= myw.MywVMEEntry(self.tl, label="RND1 rate:", vb=self.vb,
        helptext=myw.frommsRandomHelp, vmeaddr= "RANDOM_1", 
        userupdate=myw.frommsRandom)
      self.srbe= myw.MywVMEEntry(self.tl, label="RND2 rate:", vb=self.vb,
        helptext=myw.frommsRandomHelp, vmeaddr= "RANDOM_2", 
        userupdate=myw.frommsRandom)
  return(rg(vb))
def CTP_Emulator(vb):
  class CtpEmulator:
...
The errors, forced by skipping the set of these signals and messages:
PP      -prepulse
L0      -L0 signal
L1      -L1 signal
L1M     -L1 message
...
----
L1classes and L2classes are: max. 50 bits long hex. string e.g.:
---------     ---------
   0x3123456789012    classes: 50,49,45,...,5,2
   0x3                classes: 2,1
...
roc=N    N is decimal number 0-15 representing four Readout control
         bits. From codes 8-15 (which are reserved for trigger/daq 
         subsystem) following codes were already allocated:
         
         14 -for Start Of Data event (together with L1SwC, L2SwC flags)
         15 -for End Of Data event   (together with L1SwC, L2SwC flags)

ESR      Enable Segmented Readout (part of ROI option)
CIT      Calibration trigger
...
interval""")
    ...
    def startCmd(self):
      #if... self.setbcScaling()
      #if... self.setrndrate()
      rc= self.vb.io.execute("SLMstart()",applout="<>")
      #self.thdqc= self.vb.io.execute("SLMwaitemuend(1000000)",
      #  applout="<>",ff= self.quitCmdButs)
      #self.vb.io.thds[self.thdqc].plock(1)
      #print "rc[]:",rc
  ...
  class st:
    def __init__(self, vb):
      ...
      itemsB=(
        ...
        ("L1 Strobe (from meulator, before delay)","15",self.selectAB),
        ("L2 Data (from meulator, before delay)","16",self.selectAB),
...
and create human readable file""", state=NORMAL,
      ...
      self.Sshow=myw.MywButton(self.fsq, label="Show",
        helptext="Browse human readable file", state=NORMAL,
