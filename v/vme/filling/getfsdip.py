...
# operation:
# filling/linux/fill2file      -retrieve .dip file 
#   infoLogger error message
# lhc2ctp.py
#   - create $dbctp/fs_auto/fs_name.alice
#   - create $dbctp/VALID.BCMASKS
# set new coll. schedule -colschedule.bash (only if auto is chhosen)
# error/info messages
...
def main(action):
  ...
  if rc==0:
    ...
    if action=="test":
      ...
      mylog.infolog("CTPRCFG/CS not updated (update not required in ACT). CS and masks left in fs_auto/")
    elif action=="auto":
