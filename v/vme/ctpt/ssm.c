...
if(((0x7&opmo)!=SSMomvmer && (0x7&opmo)!=SSMomvmew)&&((bcstatus&0x3)!=0x2)) {
  printf("ERRO: BC signal not connected\n");/* BC not necessary for vme R/W*/
...
w32 cntval1,cntval2,loops, secs1,mics1,secs2,mics2,diffusecs,incr;
w32 cntmem1[NCOUNTERS]; w32 cntmem2[NCOUNTERS];  //the all counters version
int rc=10;
GetMicSec(&secs1, & mics1);
//cntval1= getCounter(board, cntpos); //the 'current board counters only' version
readCounters(cntmem1, NCOUNTERS, 0); cntval1=cntmem1[cntpos];
for(loops=0; loops<maxloops; loops++) {
  //cntval2= getCounter(board, cntpos);
  readCounters(cntmem2, NCOUNTERS, 0); cntval2=cntmem2[cntpos];
...
w32 ssmoffset,d, ssma,words; int i,rc, vsp;
w32 *array;
