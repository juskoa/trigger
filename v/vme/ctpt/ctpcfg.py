...
""")
  ...
  def readShared(self):
    ...
    # get first (lasthgrp1+1)=12 values of shared resources from hw
...
L or l    -means '0' (i.e. don't care)
H or h    -means '1' (i.e. BC is masked = signal is vetoed)
...
class Klas(Genhw):
  ...
  l12allvetos=[100,101,102,103,104,200,201,202,203]   # PF1-4, RoIflag,PF1-4
  l0inputs=24
  l0vetosx0=myw.Kanvas.bitWidth*l0allinputs + Ctpconfig.l0x0 + myw.Kanvas.interspace
  # 15 -2 vetos overwritten
  l0scalerx0= l0vetosx0+ myw.Kanvas.bitWidth*10 + myw.Kanvas.interspace
  ...
  def doInputs(self, inps=None):
    ...
    for cabi in range(Klas.l0allinputs)+range(100,124)+range(200,212):
      ...
      if self.clnumber>=45 and i<Klas.l0inputs:   #ok for l1,l2 too
  ...
  def refreshL0bit(self,ibit):
    ...
    if inputs & bitn==0:
      if self.clnumber>=45 and bitnx<Klas.l0inputs:  #ok for L1,2 too
        if inverted & bitn:
      ...
      for focon in fo.cons:
        ...
        for clstr in range(0,7):
          if focon.clusters & (1<<clstr):
            ...
            print "bsyinp:", detn, bsyinp
            #self.refreshClusters(detn, clstr)
            self.refreshClusters(int(bsyinp), clstr)
            self.hwwritten(0)
...
class AttrBCmask(Attr):
  ...
  def __init__(self,*fixedarg, **kw):
    ...
    self.bcmbit=int(self.atrname[3])-1   #BCM1-4
  ...
  def modmask(self, event, bn):
    print "modmask:",bn," is:", self.cci.bcmasks[bn]
