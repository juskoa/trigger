...
//FGROUP Configuration GUI LVDSTfun
/*FGROUP TOP GUI LVDSTfun
Daniel's stuff
*/

/*FGROUP Syn GUI LVDSTopt
Daniel's stuff
*/
...
int i, number, k, arrayCableErrors[33],arraySequenceStrobes[33];
float arrayRatio[33];
kTp = Tp_transition(1);
kTs = Ts_transition(1);
...
for (k=0; k<=32; k++) {
	...
	// Cable1-errors
        arrayCableErrors[k]=mystatus[8];
	//Number of sequence strobes - number of patterns generated
        arraySequenceStrobes[k]=mystatus[11];
...
//printf("Total number of settings were erros were found %d\n",count);
}
/*FGROUP SimpleTests
