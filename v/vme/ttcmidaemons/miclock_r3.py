...
if os.environ['VMESITE']=='ALICE':
  ...
  MICLOCKID="/home/trigger/v/vme/WORK/miclockid"
...
1:(0,'NO MODE'),    # LOCAL
2:(0,'SETUP'), 
3:(0,'INJECTION PROBE BEAM'),
4:(0,'INJECTION SETUP BEAM'), 
5:(0,'INJECTION PHYSICS BEAM'), 
6:(0,'PREPARE RAMP'), 
7:(1,'RAMP'), 
...
def checkandsave(csf_string, fineshift="None", force=None):
  ...
  if force != None:
    ...
    arg=("none",)
    mylog.logm("DLL_RESYNC not done.")
    #mylog.logm("DLL_RESYNC after automatic clock shift adjustement...")
...
def callback_bm(ecsbm):
  ...
  if (bmname=="PREPARE RAMP") or (bmname=="RAMP"):
    ...
    if True:   #os.environ['VMESITE'] != "ALICE":
      pass # mylog.logm("lab env, 'getfsdip.py act' call skipped")
    else:
      import getfsdip
      reload(getfsdip)
    ...
    if bmname=="FLAT TOP":
      ...
      if cshift!='old':
        if False:
          ...
          mylog.infolog("FLAT TOP: clock shift %s ns, applying correction ..."%cshift,level='w')
...
"""%(pid,pid,pid,MICLOCKID))
  ...
  udpmon_thread.start()
  if stdinp==False:
    alivefp= os.path.join(os.environ['VMEWORKDIR'], "WORK/miclock.alive")
    print("started?: nohup ./miclock.py & >~/v/vme/WORK/miclock.nohup &")
    while True:
      time.sleep(60) 
      with open(alivefp,"a"):
        os.utime(alivefp, None)
  while True:
    #time.sleep(10)
   ...
   dllresync    -in case clock not stable (FT0) (during PbPb)
