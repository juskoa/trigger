...
int rcscp; char cmd[100];
 ...
 stopSSM(1);   // which is coming 19 orbits before interaction
...
int printCounters(w32 *l0, int trig, FILE *f){
 ...
 curtime= l0[L0TIMEOFFSET]; deltime=dodif32(prevtime, curtime); 
 if(trig>24) {
 ...
 */
 while(1){
    ...
    if(trigcond){
      ...
      getSSMs(inpnum,intboard,f);   // to be checked: what happens if inpnum=0?
      trigold=trig;
      //countersRead();
      //break;
    };
    ...
    if(timediff>1000.*2){   // per 1 secs
      //char dt[32];
      //printCounters(L0counts, inpnum, f); 
      //getdatetime(dt);
      //dt[10]='_';
