...
BCM_NUMBER=4        # 4 before AC, 12: firmAC
if BCM_NUMBER==4:
...
def Parse1(clstring):
  ...
  if len(nameits)>1:
    if nameits[1][-1]!=')':
      PrintError(") missing in line:"+clstring)
    else:
      nameits[1]= nameits[1][:-1]  # stripp ')'
     ...
     #print "oe:",oe,"vars:",vars
      ...
      if L0F_NUMBER==4:
        ...
        if inp.name[:3]=='l0A':
          ...
          maxL0F_NUMBER= 24 ; minL0F_NUMBER=13
        else:
          maxinputs= 4
          maxL0F_NUMBER= 4
      ...
      for ixl0f in range(len(vars)):
        ...
        if inl0f==None:
          ...
          PrintError("""%s is not (allowed) L0 input in l0fun:%s=%s right now. 
%d <= %d <= %d
.pcfg file will not be written (or incorrect if written) 
if this descriptor is used with any class in this partition.
"""% (inl0f.name, inp.name, inp.l0fdefinition,
  minL0F_NUMBER, inl0f.ctpinp[1],maxL0F_NUMBER), self)
          err_rc= 5
        else:
...
%s discarded.  """%(inpname, line), self)
      ...
      if (line[:3]=='l0f') or (line[:3]=='l0A') or (line[:3]=='l0B'):
        #print "l0fun* special function in VALID.CTPINPUTS"
        detname= "L0FUNS"
      else:
...
class TrgClass:
    ...
    for k in pars.keys():
      ...
      if k[:3]=='BCM':
        ...
        if (bcmix>=1) and (bcmix<=12):
          ...
          print "Bad BCM%s in:"%bcmix,clstring
          break
      if k[:2]=='PF':
        bcmix= int(k[2:])   # 1..4
        if (bcmix>=1) and (bcmix<=4):
          ...
          print "Bad PF%s in:"%bcmix,clstring
          break
      if k=='all':
  ...
  def getTXTpfs(self):
    bcms=''
    for ibcm in range(len(self.pfs)):        # bcms
      if self.pfs[ibcm]!=0:
        bcms=bcms+"PF%d,"%(ibcm+1)
    return bcms
  def getPFs(self):
    pfs='{'   # get P/Fs:
    if self.pfs!=None:
...
"""
    ...
    von="""
    onoffs=[];
    helptextma="BC masks. 4/12 possible masks can be choosen simultaneously: BCM1 BCM2 BCM3 or BCM4 .. BCM12 "
    for ixoi in range(len(self.bcms)):
      if self.bcms[ixoi]==1:
        onoffs.append(1)
      else:
        onoffs.append(0)
    self.maskbut=myw.MywMenuList(self.tlw,
      label="BCmasks", cmd=self.modadi, showactive=1,
      items=['BCM1','BCM2','BCM3','BCM4','BCM5','BCM6','BCM7','BCM8','BCM9','BCM10','BCM11','BCM12'], defaults=onoffs, side=TOP,
      helptext=helptextma)
"""
    # rare
...
"""+TDLTUS.tdshelptext
  ...
  def show(self,master):
    ...
    self.tdsfr= myw.MywFrame(self.clfr)
    self.tdsheadfr= myw.MywFrame(self.tdsfr,side=LEFT)
...
Logical class """+str(clanum)+", cluster:"+cluster.name+", class name:"+ cls.getclsname())
  ...
  def savercfg(self, line=""):
    ...
    print "Applying mask:", hex(detmask)
    ...
    #prepare 3 sections: CLASSES DESCRIPTORS INPUTS(directly written)
    for ixclu in range(len(self.clusters)):
      ...
      for cla in cluster.cls:
        ...
        if useddescriptors.has_key(desname): continue
        #print "cla.trde:"
        #cla.trde.trdprt()
        line=''
...
"""
    ...
    for ixm in range(BCM_NUMBER):
      #print "bcm%d"%i,SHRRSRCS[BCMASKS_START+ixm].getDefinition()
      ...
      for cls in cluster.cls:
        for ix in (0,1):
          ...
          if trdel0funs[ix]:   # l0f[ix] used
            ...
            #von lse:    lutval=          trdel0funs[ix][0]
...
"""
    ...
    while 1:
      ...
      if string.find(cltds, 'Version:')==0:   # has to be first line
        self.version= string.strip(string.split(cltds,':')[1])
        continue
...
def main():
  global partw
  """
  parted.py partname        -open GUI to edit partition
