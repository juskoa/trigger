...
import os, os.path, glob, types, time
import myw,counters
...
Actually, the interval between triggers can be much more higher 
...
Exceptions:
Value 0: generate START signal as fast as possible
>1000000: time between 2 triggers is: (1000000-spacing)us""")
      #
...
""",
    ...
    def doSeqNames(self):
      ...
      #os.chdir(cwd);
      names= self.vb.io.execute('prtfnames("'+self.slmdir+'",".seq")',"no").split()
      names= names+ self.vb.io.execute('prtfnames("'+self.slmdir+'",".slm")',"no").split()
...
-type path relative to VMEWORKDIR, full name (including .seq or .slm suffix)""", 
    ...
    def filedownload(self, inst, ix):
      if ix>0 and self.vb.boardName in myw.DimLTUservers:
...
Error on demand can be generated in parallel with random errros."""
    ...
    def swStart(self):
      if self.sw2generate>0:
        ...
        if (int(spacing)<1000000) and \
          (int(spacing)>=100 or int(n)*(int(spacing)+1) > 2000):
...
interval""")
    ...
    def loadseq(self):
      ...
      if fnbase=='' or fnbase==' ' or fnbase==self.CLICKHERE or len(fnbase.split('.'))<2:
        self.vb.io.write('Choose right sequence to be loaded first...\n')
        return
      fn= os.path.join(self.slmdir,fnbase)
      if fnbase.split('.')[1]=='slm':   # new format, compile first
        import slmcmp
        if self.vb.lturun2:
          run="-run2"
        else:
        ...
        if self.vb.boardName in myw.DimLTUservers:
    ...
    def seqeditor(self):
      ...
      if len(fnbase.split('.'))==1:   # new format
        # .slm default:
        #os.system("cp -u CFG/ltu/SLM/all.slm CFG/ltu/SLM/newslm.slm")
        #fnbase='newslm.slm'
        # .seq default:
        fnbase='blabla.seq'
      fname=fnbase.split('.')[0]
      if fnbase.split('.')[-1]=='slm':   # slm format
      ...
      if snbix==0:
        ...
        snb= self.itn[snbix][0].split('.')[0]
      cmd= "(cd "+self.slmdir+"; "+ltu6path+" "+snb+")"
      #print cmd
      #os.system(cmd+' &')
      popio= os.popen(cmd, "r")
      while 1:
        wfn= popio.readline().rstrip()
        #print "popio:",wfn
        if wfn=="": break
        fn=wfn.split(":")
        if len(fn)==2:
...
class Editor(object):
  ...
  def save(self):
    ...
    OpenSave=tkinter.filedialog.SaveAs(
...
#        cnts.addReg(i)
  print(vb.io.execute("getgltuver()",applout="<>"))
