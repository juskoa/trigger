...
class ioWindow(object):
  ...
  def getOutput(self, wout='out', applout='out'):
    ...
    while 1:
        ...
        if applout =="out" or applout=="<>":
          #lastOutput=lastOutput+str(line)
    ...
    #  print(sys.exc_info())
    #  #print('exc_value:',etype,evalue,etrace)
    if applout =="out":
      return lastOutput
    elif applout=="<>":
  ...
  def __init__(self, cli,ixthds):
    ...
    if not os.path.exists(exep):
      ...
      #self.io[1].flush()   # necessarey after io[1].write
      #print("cmdlin2.popen ok")
...
def main():
  ...
  f1= myw.getCrate(f)
  if len(sys.argv)>1:
    vmeboard= myw.VmeBoard(f1, boardName="ltu", \
                baseAddr="0x810000") #, initboard=initboard)
    f1.addBoard(vmeboard)
    #syspadd= os.path.join(cfdir, "ltu")
    #vmeboard.openCmd(); myw.vbnit(vmeboard)
    c= cmdlint(sys.argv[1], board=vmeboard)
  else:
